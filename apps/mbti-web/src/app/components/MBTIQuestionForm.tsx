  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mt-4">
        <p className="text-sm text-gray-600">{questionId + 1}/4 </p>
        <button
          onClick={() =>
            router.push(`/question/${Math.max(0, questionId - 1)}`)
          }
          disabled={questionId === 0}
          className="px-4 py-2 border border-[#fd9340] text-[#fd9340] rounded-xl hover:bg-[#fd9340] hover:text-white transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          이전으로
        </button>
      </div>
